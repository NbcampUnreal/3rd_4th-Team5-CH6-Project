// GC_Material_Mummy.cpp
#include "GAS/GC/Gimmick/GC_Material_Mummy.h"

#include "Components/SkeletalMeshComponent.h"
#include "Materials/MaterialInstanceDynamic.h"
#include "TimerManager.h"

DEFINE_LOG_CATEGORY_STATIC(LogGCMaterialMummy, Log, All);

AGC_Material_Mummy::AGC_Material_Mummy()
{
	// 기본값 설정 (에디터에서 변경 가능)
	OriginalMaterialPath = FSoftObjectPath(TEXT("/Game/ThirdParty/CardboardWarrior/Materials/Mat_Cardboard_warrior_Body"));
	SkinMaterialPath = FSoftObjectPath(TEXT("/Game/ThirdParty/Cardboard_Boy_SkinPack/Materials/Mat_Cardboard_warrior_Body_skin2"));
	FadeDuration = 0.5f;
...
bool AGC_Material_Mummy::OnActive_Implementation(AActor* MyTarget, const FGameplayCueParameters& Parameters)
{
	if (!MyTarget)
	{
		UE_LOG(LogGCMaterialMummy, Warning, TEXT("OnActive - MyTarget가 null입니다."));
		return false;
	}
	
#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
	UE_LOG(LogGCMaterialMummy, Log, TEXT("OnActive - 타겟: %s"),
		*MyTarget->GetName());
#endif
	...
	if (!SkinMaterial)
	{
		UE_LOG(LogGCMaterialMummy, Error,
...
bool AGC_Material_Mummy::OnRemove_Implementation(AActor* MyTarget, const FGameplayCueParameters& Parameters)
{
	if (!MyTarget)
	{
		UE_LOG(LogGCMaterialMummy, Warning, TEXT("OnRemove - MyTarget가 null입니다."));
		return false;
	}
	
#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
	UE_LOG(LogGCMaterialMummy, Log, TEXT("OnRemove - 타겟: %s"), *MyTarget->GetName());
#endif
...
void AGC_Material_Mummy::ExecuteFadeIn(AActor* TargetActor, UMaterialInterface* SkinMaterial)
{
	...
	if (FMaterialFadeInfo_Mummy* ExistingInfo = ActorFadeInfoMap.Find(TargetActor))
	{
	...
	FMaterialFadeInfo_Mummy FadeInfo;
	...
	GetWorldTimerManager().SetTimer(
		...
		[this, TargetActor, ElapsedTime]() mutable
		{
			...
			FMaterialFadeInfo_Mummy& Info = ActorFadeInfoMap[TargetActor];
...
#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
				UE_LOG(LogGCMaterialMummy, Log, TEXT("Fade In 완료: %s"), *TargetActor->GetName());
#endif
			}
		},
		0.016f,
...
#if UE_BUILD_DEBUG || UE_BUILD_DEVELOPMENT
	UE_LOG(LogGCMaterialMummy, Log, TEXT("Fade In 시작: %s"), *TargetActor->GetName());
#endif
}

void AGC_Material_Mummy::ExecuteFadeOut(AActor* TargetActor)
{
	if (!TargetActor)
	{
		return;
	...
	FMaterialFadeInfo_Mummy* FadeInfoPtr = ActorFadeInfoMap.Find(TargetActor);
	if (!FadeInfoPtr)
	{
		UE_LOG(LogGCMaterialMummy, Warning,
