...
void ATSPlayerCharacter::PossessedBy(AController* NewController)
{
	...
	// 매개변수: (어빌리티 클래스, 어빌리티 레벨, 입력 ID, 소스 오브젝트)
	FGameplayAbilitySpec Spec(UTSGA_TEST_ReceiveGameplayEvent::StaticClass(), 1, INDEX_NONE, this);
	FGameplayAbilitySpecHandle Handle = ASC->GiveAbility(Spec);

	if (Handle.IsValid())
	{
		UE_LOG(LogTemp, Log, TEXT("[TS_Debug] Ability Granted Successfully! Handle: %s"), *Handle.ToString());
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("[TS_Debug] Ability Grant FAILED!"));
	}
...
void ATSPlayerCharacter::OnRep_PlayerState()
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[TS_Debug] PlayerState is NULL!"));
		return;
	}
	...
	if (!IsValid(ASC))
	{
		UE_LOG(LogTemp, Error, TEXT("[TS_Debug] ASC is NULL!"));
		return;
	}
	...
	Payload.EventMagnitude = 99.f; 
	Payload.EventTag = InputTag::LeftClick::TS_Input_LeftClick_Default;
	
	UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(this, InputTag::LeftClick::TS_Input_LeftClick_Default, Payload);
	
	UE_LOG(LogTemp, Log, TEXT("[TS_Debug] PlayerState Replicated!"));
