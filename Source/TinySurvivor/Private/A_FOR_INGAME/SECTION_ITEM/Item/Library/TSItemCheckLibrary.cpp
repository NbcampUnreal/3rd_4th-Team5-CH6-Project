...
bool UTSItemCheckLibrary::IsThisItemValid_Lib(const UObject* InWorldContextObject, FTSItemRuntimeData& InItemRuntimeData)
{
	// 아이템 데이터 매니저로부터 정적 데이터 포인터 유효한지 체크
	UTSItemDataSubSystem* NewItemDataSubSystem =  UTSItemDataSubSystem::Get(InWorldContextObject);
	if (!IsValid(NewItemDataSubSystem)) return false;
		
	FTSITemStaticData* FouNewITemStaticData = NewItemDataSubSystem->GetItemStaticData(InItemRuntimeData.StaticDataID);
...
bool UTSItemCheckLibrary::IsThisITemCanStack_Lib(const UObject* InWorldContextObject, FTSItemRuntimeData& InItemRuntimeData)
{
	// 아이템 데이터 매니저로부터 정적 데이터 포인터 유효한지 체크
	UTSItemDataSubSystem* NewItemDataSubSystem =  UTSItemDataSubSystem::Get(InWorldContextObject);
	if (!IsValid(NewItemDataSubSystem)) return false;
...
int32 UTSItemCheckLibrary::GetMaxStackSize_Lib(const UObject* InWorldContextObject, FTSItemRuntimeData& InItemRuntimeData)
{
	// 아이템 데이터 매니저로부터 정적 데이터 포인터 유효한지 체크
	UTSItemDataSubSystem* NewItemDataSubSystem =  UTSItemDataSubSystem::Get(InWorldContextObject);
...
bool UTSItemCheckLibrary::IsThisItemCanPlaceInThisInventory_Lib(const UObject* InWorldContextObject, FTSItemRuntimeData& InItemRuntimeData, FTSInventoryCompData& InInventoryData)
{
	// 아이템 데이터 매니저로부터 정적 데이터 포인터 유효한지 체크
	UTSItemDataSubSystem* NewItemDataSubSystem =  UTSItemDataSubSystem::Get(InWorldContextObject);
	if (!IsValid(NewItemDataSubSystem)) return false;
...
bool UTSItemCheckLibrary::IsThisSameItem_Lib(FTSItemRuntimeData InFromSlotItemData, FTSItemRuntimeData InItemRuntimeData)
{
