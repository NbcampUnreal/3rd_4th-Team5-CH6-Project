...
int32 UTSInventoryInternalHelperLibrary::FindEmptySlot_Lib(const UObject* InWorldContextObject, FTSInventoryCompData& InInventoryCompData, FTSItemRuntimeData& InItemRuntimeData)
{
	// 인벤토리 배열 체크 
	...
	for (int32 Index = 0; Index < InInventoryCompData.InventoryDataArray.Num(); ++Index)
	{
		...
		FTSInventorySlotData& SlotData = InInventoryCompData.InventoryDataArray[Index];
		
		// 이미 아이템이 있으면 건너뛰기
		if (UTSInventorySlotInternalHelperLibrary::IsSlotEmpty_Lib(SlotData) == false) continue;
...
int32 UTSInventoryInternalHelperLibrary::FindStackSlot_Lib(const UObject* InWorldContextObject, FTSInventoryCompData& InInventoryCompData, FTSItemRuntimeData& InItemRuntimeData)
{
	// 인벤토리 배열 체크 
	if (InInventoryCompData.InventoryDataArray.IsEmpty()) return -1;
	...
	for (int32 Index = 0; Index < InInventoryCompData.InventoryDataArray.Num(); ++Index)
	{
		...
		// 비었으면 건너뛰기 
		if (UTSInventorySlotInternalHelperLibrary::IsSlotEmpty_Lib(SlotData) == true) continue;
...
bool UTSInventoryInternalHelperLibrary::AddItemToEmptySlot_Lib(const UObject* InWorldContextObject, FTSInventorySlotData& InTargetSlot, FTSItemRuntimeData& InItemRuntimeData)
{
 	// 1. 슬롯의 데이터 통째로 복사
...
bool UTSInventoryInternalHelperLibrary::AddItemToStackSlot_Lib(const UObject* InWorldContextObject, FTSInventorySlotData& InTargetSlot, FTSItemRuntimeData& InItemRuntimeData)
{
	// 남은 스택 수와 최대 스택 수 
	int32 RemainStack = 0;
