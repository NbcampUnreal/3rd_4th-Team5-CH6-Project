...
public:
	...
	bool AddItem(FName StaticDataID, FName DynamicDataID, int32 Quantity);
...
private:
	...
	/** 아이템 슬롯 정보 가져오기 (아이템 컨테이너 매니저에서 조회) */
	FItemInventoryAndSlotInfo GetItemSlotInfo(FName StaticDataID) const;
	
	/** 장착된 가방 아이템의 StaticDataID 가져오기 */
	FName GetEquippedBagItemID() const;
	
	/** 특정 가방 아이템의 슬롯 개수 가져오기 */
	int32 GetBagSlotCount(FName BagItemID) const;
};