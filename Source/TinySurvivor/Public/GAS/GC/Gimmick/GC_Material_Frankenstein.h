...
#include "GC_Material_Frankenstein.generated.h"

// 액터별 머티리얼 페이드 정보를 저장하는 구조체
USTRUCT()
struct FMaterialFadeInfo_Frankenstein
{
	GENERATED_BODY()
...
/*
	"Frankenstein" 형태로 머티리얼 변환 및 페이드 효과를 처리하는 GameplayCue
	
	- 특정 액터의 머티리얼을 "Frankenstein" 형태로 변경하고, 페이드 효과 적용
...
*/
UCLASS()
class TINYSURVIVOR_API AGC_Material_Frankenstein : public AGameplayCueNotify_Actor
{
	GENERATED_BODY()
	
public:
	AGC_Material_Frankenstein();
...
private:
	// 액터별 페이드 정보 저장
	UPROPERTY()
	TMap<TWeakObjectPtr<AActor>, FMaterialFadeInfo_Frankenstein> ActorFadeInfoMap;
